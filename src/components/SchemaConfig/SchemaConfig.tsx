...
// import { Plus, Trash2, Sparkles, Upload, Settings, Calendar, Hash, DollarSign, Percent, List, Layers, Package } from 'lucide-react'
...
//           {schemas.map((schema) => (
//             <Card key={schema.id} className="hover:shadow-lg transition-shadow duration-200">
...
//                           )}
//                         </div>
//                         <span className="text-muted-foreground capitalize">{field.type}</span>
//                       </div>
//                     ))}
              ...
              <CardContent className="pt-0 flex-1">
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    <h4 className="text-sm font-medium text-foreground">Fields Preview</h4>
                    <div className="space-y-1 max-h-32 overflow-y-auto">
                      {schema.fields.slice(0, showAllFields[schema.id] ? schema.fields.length : 4).map((field, index) => (
                        <div key={index} className="flex items-center justify-between text-xs bg-muted/50 rounded px-2 py-1">
                          ...
                          <span className="text-muted-foreground capitalize text-xs ml-2 flex-shrink-0">
                            {field.type}
                          </span>
                        </div>
                      ))}
                    ...
                    {schema.fields.length > 4 && (
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => toggleShowAllFields(schema.id)}
                        className="w-full text-xs h-6"
                      >
                        {showAllFields[schema.id] ? (
                          <>
                          ...
                          <>
                            ...
                            Show All ({schema.fields.length - 4} more)
                          </>
                        )}
                      </Button>
                    )}
